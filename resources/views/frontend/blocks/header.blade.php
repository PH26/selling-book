  ...
  <div class="notifyjs-wrapper notifyjs-hidable">
    ...
    <!-- header top -->
      ...
      <div class="header-bottom">
        <div class="container">
          <div class="row">
            <div class="col-md-4 col-sm-12 header-logo"> <a href="{!! url('/') !!}" title="Bookstore"> <img src="{!! asset('upload/logoweb/logo-white.png') !!}" width="275" height="39" /> </a> </div>
              ...
              <nav class="header-nav">
                <ul>
                  ...
                  <li class="@yield('sanpham')"> <a href="{!! url('san-pham') !!}">Sản phẩm <i class="fa fa-caret-down"></i></a>
                    <ul>
                    <?php $menus = DB::table('categories')->select('id','name','prarent_id')->where('prarent_id',0)->orderBy('id','DESC')->get(); ?>
                      @foreach ($menus as $menu)
                        <li><a href="{!! url('danh-muc',[$menu->id]) !!}">{!! $menu->name !!}</a>
                          <?php $parent = DB::table('categories')->select('id','name','prarent_id')->where('prarent_id',$menu->id)->orderBy('id','DESC')->get(); ?>
                            <ul>
                              @foreach($parent as $parents)
                                <li><a href="{!! url('category',[$parents->id]) !!}"><?php echo $parents->name ?></a></li>
                              @endforeach
                            </ul>
                        </li>
                      @endforeach
